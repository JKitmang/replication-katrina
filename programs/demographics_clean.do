*===============================================================================
* FILE:     demographics_clean.do
* PROJECT:  Katrina's Children (Imberman, Kugler & Sacerdote, AER)
* ROLE:     Data cleaning
* PRODUCES: Intermediate dataset demog_new.dta (input to the master merge in merge_c.do)
*
* PURPOSE:  Appends the yearly HISD demographic files into one id-year panel,
*           harmonizes the many inconsistent variable names used across year
*           vintages (campus, id, ethnicity, gender, immigrant, bilingual, dob,
*           econ-disadvantage, gifted, special-ed), converts grade to numeric,
*           flags students whose time-invariant traits (gender/race/dob) are
*           inconsistent across years, and constructs "cleaned" most-frequent /
*           most-recent versions of those traits. Keeps all observations but
*           tags the inconsistent ones rather than dropping them.
*
* INPUTS:   demog_1993.dta .. demog_2006.dta
* OUTPUTS:  demog_new.dta
*
* KEY STEPS:
*   - Append yearly demographic files; stamp the year
*   - Reconcile renamed variables across vintages
*   - Build female indicator; numeric grade (KG=0, PK=-1, EE=-2); lunch flags
*   - Apply variable labels
*   - Flag ids with inconsistent gender / race / dob across years
*   - Backfill dob with most recent; build immigrant-ever indicator
*   - Construct most-frequent (tie -> most-recent) ethnicity_2 and female_2
*
* HARDCODED YEARS / PATHS TO UPDATE FOR A NEW-YEARS REPLICATION:
*   - cd "C:/D/Research/Charter/Houston/HISDdata/DataFiles/Demographics"  (PATH)
*   - use demog_1993 ; gen year = 1993        (YEAR HARDCODE: first file)
*   - forvalues year = 1994/2006              (YEAR HARDCODE: append range)
*   - replace id = local_id if year >= 2005   (YEAR HARDCODE: local_id used 2005+)
*   - dob backfill / female_2 loops gated on year >= 1996 (YEAR HARDCODE)
*   - forvalues t = 1/15 / 1/12 loop bounds depend on panel length
*
...
set mem 900m
set more off
cd "C:/D/Research/Charter/Houston/HISDdata/DataFiles/Demographics"   // PATH: repoint to your local globals


* ---- Append all yearly demographic files into one panel ----
*GET ALL DEMOGRAPHIC FILES INTO ONE DATASET
  use demog_1993        // YEAR HARDCODE: first demographic file
  gen year = 1993       // YEAR HARDCODE: stamp 1993

  forvalues year = 1994/2006 {   // YEAR HARDCODE: append 1994 through 2006
...
* ---- Reconcile variables that were renamed across year vintages ----
...
replace id = local_id if year >= 2005   // YEAR HARDCODE: id stored as local_id from 2005 on
drop local_id
...
* ---- Convert grade to numeric (KG=0, PK=-1, EE=-2) ----
*MAKE GRADES NUMERIC
gen temp = grade
drop grade
gen grade = real(temp)
...
* ---- Build economic-disadvantage (free / reduced / other) flags ----
...
* ---- Flag ids with inconsistent time-invariant traits (gender / race / dob) ----
*IDENTIFY IDS W/ INCONSISTENT TIME INVARIANT STATISITCS (2.4% of observations, 1.6% of id's)
*MAKE 2 DATASETS - A KEEPS ALL OBS, B DROPS IDS WITH MULTIPLE GENDERS OR RACES
gen byte  baddatid = 0
	...
	*DOB
	...
	label variable flag_dob "date of birth is not consistent across time periods for this id"
		...
		*SINCE THE MOST RECENT DOB IS LIKELY MOST ACCURATE, USE THAT ONE
		gen dob_2 = dob
		tsset (id) year
		forvalues t= 1/15 {   // YEAR HARDCODE: 15 = max panel length; widen for longer panels
			replace dob_2 = f.dob_2 if f.dob_2 != . & year >= 1996   // YEAR HARDCODE: dob only reliable 1996+
		}
	...
	* ---- Most-frequent (tie -> most-recent) ethnicity_2 and female_2 ----
	...
	*ETHNICITY
	...
	forvalues k = 1/5 {
		...
		*NOTE THAT TWO OBSERVATIOSN 3 ETHNICITIES PROVIDED OVER 3 YEARS BUT ARE FIXED BY THIS PROCEDURE
		tsset (id) year
		forvalues k = 1/12 {   // YEAR HARDCODE: 12 lead passes ~ panel length
	...
	*GENDER
	...
	forvalues k = 0/1 {
		...
		*NOTE THAT TWO OBSERVATIOSN 3 ETHNICITIES PROVIDED OVER 3 YEARS BUT ARE FIXED BY THIS PROCEDURE
		tsset (id) year
		forvalues k = 1/15 {   // YEAR HARDCODE: 15 lead passes ~ panel length
	...
	* ---- Save cleaned demographic file (input to merge_c.do) ----
	*SAVE
	sort id year
	compress
