*===============================================================================
* FILE:     epple_romano_f_ela_grade_fullint_nolagint.do
* PROJECT:  Katrina's Children (Imberman, Kugler & Sacerdote, AER)
* ROLE:     Robustness/variant (Epple-Romano test, ELA, "lag not interacted")
* PRODUCES: "Lag not interacted" robustness rows of the Epple-Romano linear-in-
*           means results (Table 6 / Appendix Table 14, ELA) -- middle/high only.
*
* PURPOSE:  ELA/reading counterpart of epple_romano_f_math_grade_fullint_nolagint.do.
*           Same Epple-Romano construction (peer MEAN achievement by own quartile,
*           instrumented by evacuee CONTRIBUTION to the lagged peer mean), but it
*           runs ONLY the middle/high (grade>5, lag sample) "with lag" rows and
*           enters the lagged-score terms WITHOUT interacting them with the own
*           quartile (the `xi i.elaQUART*ela_lag_*` step present in the main file is
*           omitted here). The elementary block and the "no lag" rows are dropped.
*
...
* OUTPUTS:  regression/test output to the log (no temp_*.dta is saved here).
*
* KEY STEPS:
...
*   - Keep mid/high lag sample, demean by campus*quartile, OLS / RF / 2SLS w/lag.
*
...
* ===========================================================================
* ---- EPPLE-ROMANO TEST CONSTRUCTION (ELA, "lag not interacted" variant) ----
...
* ===========================================================================
* ---- MIDDLE/HIGH (grade>5, lag sample) ONLY: OLS / reduced form / 2SLS ----
* "lag not interacted" variant: only the with-lag rows; lag NOT interacted
* with own quartile (no elementary block, no no-lag rows).
* ===========================================================================
****MIDHIGH****
...
* 2SLS w/ lag controls NOT interacted (lag-not-interacted variant, mid/high ELA) -> Table 6 / App 14
*MIDHIGH W/LAG
