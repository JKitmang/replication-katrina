*===============================================================================
* FILE:     epple_romano_f_math_grade_fullintb.do
* PROJECT:  Katrina's Children (Imberman, Kugler & Sacerdote, AER)
* ROLE:     Analysis (Epple-Romano linear-in-means peer-effect test, MATH)
* PRODUCES: Table 6 and Appendix Table 14 (math panel) -- the Epple-Romano
*           correct test for the linear-in-means model of peer effects.
*
* PURPOSE:  Implements the Epple-Romano (2011)-style test of the linear-in-means
...
*           OLS, reduced-form, and 2SLS specifications separately for elementary
*           (grade<=5) and middle/high (grade>5, lag sample) and tests equality
*           of the peer-mean coefficients across the four own-achievement
*           quartiles.
*
...
* OUTPUTS:  temp_math.dta (intermediate); regression/test output to the log
...
*   - Build within grade-year baseline achievement quartiles (math0005 -> mathQUART).
...
*   - replace math_lag = math0005 if year>2005 (lag forced pre-Katrina).
...
*   - math2003/math2004 baseline-score references; lagyear capped at min(.,2004).
...
bysort grade_num year: quantiles math0005, gen(mathQUART) nq(4) stable   // baseline achievement quartiles (math)
...
* ===========================================================================
* ---- EPPLE-ROMANO LINEAR-IN-MEANS TEST CONSTRUCTION (MATH) ----
...
foreach var of varlist free_lunchA male black hisp asian gryr* {
    local x = `x' + 1
    xi i.mathQUART*`var', prefix(_g`x')
}
...
* 2SLS (headline Epple-Romano IV, elementary math) -> Table 6 / App Table 14
...
foreach var of varlist  math_lag_* {
    local x = `x' + 1
    xi i.mathQUART*`var', prefix(_l`x')
}
...
*MIDHIGH W/LAG
reg mathSTD math_lag_*  peer_mean_Q1 peer_mean_Q2 peer_mean_Q3 peer_mean_Q4  free_lunchA male black hisp asian gryr* _g* _l*   if grade_num>5 & lagsample==1 & mathQD1 != .,  cluster(sitecode)
...
*MIDHIGH W/LAG
reg mathSTD math_lag_*  contribution_katrina_Q1 contribution_katrina_Q2 contribution_katrina_Q3 contribution_katrina_Q4  free_lunchA male black hisp asian gryr* _g* _l*  if grade_num>5 & lagsample==1 & mathQD1 != . & peer_mean_Q1 != .,  cluster(sitecode)
...
* 2SLS w/ lagged-score controls (headline IV, mid/high math) -> Table 6 / App Table 14
*MIDHIGH W/LAG
ivreg mathSTD math_lag_*  (peer_mean_Q1 peer_mean_Q2 peer_mean_Q3 peer_mean_Q4 = contribution_katrina_Q1 contribution_katrina_Q2 contribution_katrina_Q3 contribution_katrina_Q4 )   free_lunchA male black hisp asian gryr* _g* _l*   if grade_num>5 & lagsample==1 & mathQD1 != .,  cluster(sitecode)
