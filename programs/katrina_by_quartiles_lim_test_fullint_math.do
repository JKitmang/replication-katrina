*===============================================================================
* FILE:     katrina_by_quartiles_lim_test_fullint_math.do
...
* PRODUCES: MATH columns of the Houston linear-in-means tables (Table 4 and
...
*           incumbents' MATH achievement. Following Hoxby-Weingarth (2005), the
*           evacuee share in each native pre-Katrina quartile is interacted with
*           the native student's own baseline quartile, and average peer score is
*           instrumented with the evacuee (Katrina) share. If peer effects are
*           truly linear-in-means, the four quartile-specific peer-score
...
* ---- Fully interact every control (incl. lag) with native math quartile (_g*) ----
local x 1
...
* ---- Estimation: OLS / reduced-form / 2SLS, with cross-quartile equality test ----
	...
	*OLS  (math peer-score coefficients by native quartile; test = LIM check)
