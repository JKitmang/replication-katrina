*===============================================================================
* FILE:     katrina_by_quartiles_lim_test_fullint_math_nolagint.do
...
* PRODUCES: MATH "lag-not-interacted" robustness rows of the Houston linear-in-
*           means tables (Appendix Tables 9 & 10; companion to Table 4).
*
* PURPOSE:  Same "limited test" of the linear-in-means peer-effect model for
*           HISD incumbents' MATH achievement as fullint_math.do, but the lagged
*           baseline-score control is KEPT and entered ONLY as a single level
*           term (ltaks_sd_min_math), NOT fully interacted with native quartiles.
*           Peer score (by quartile) is instrumented with the evacuee share (by
*           quartile); the `test` commands check equality of the four quartile-
*           specific peer coefficients (the LIM restriction).
*           VARIANT: "_nolagint" = lag included but NOT quartile-interacted.
*           (NOTE: the in-body "*READ" label is a copy-paste leftover; this file
*            operates on MATH throughout.)
*
...
*   - Within campus x quartile, center variables (lag terms included in centering).
*   - Run OLS / reduced-form / 2SLS with the lag entered as a single level term.
*
...
* ==== MATH dependent variable (in-body "*READ" label is a copy-paste leftover) ====
	*READ

  // keep only natives with a non-missing lagged baseline math score (lag control present)
...
* ---- Within-campus x quartile centering (lag terms included in centering) ----
sort campus taks_sd_min_math_quartile
by campus taks_sd_min_math_quartile: center  taks_sd_min_math taks_sd_min_math_peer_*  _k*  katrina_frac_grade_math_Q* ltaks_sd_min_math_*, replace

foreach var of varlist   taks_sd_min_math taks_sd_min_math_peer_*  _k*  katrina_frac_grade_math_Q* ltaks_sd_min_math_* {
  replace `var' = c_`var'
}

* ---- Fully interact controls AND lags with quartile (_g*); lag enters level-only below ----
local x 1
foreach var of varlist _k* ltaks_sd_min_math_* {
    local x = `x' + 1
...
* ---- Estimation: lag entered as single level term (ltaks_sd_min_math), NOT interacted ----
        *QUARTILES

	*OLS  (math peer-score coefficients by native quartile)
	reg taks_sd_min_math taks_sd_min_math_peer_* _k* ltaks_sd_min_math  _g*, cluster(campus) nocons
	...
	*REDUCED FORM  (evacuee share by quartile -> outcome)
	reg taks_sd_min_math katrina_frac_grade_math_Q*   _k*  ltaks_sd_min_math  _g* if taks_sd_min_math_peer_Q1 != ., cluster(campus) nocons
	...
	*SECOND STAGE  (2SLS: peer score instrumented by evacuee share, by quartile)
	ivreg taks_sd_min_math  (taks_sd_min_math_peer_Q* = katrina_frac_grade_math_Q*)  _k* ltaks_sd_min_math _g*, cluster(campus) nocons
	test taks_sd_min_math_peer_Q1 = taks_sd_min_math_peer_Q2 = taks_sd_min_math_peer_Q3 = taks_sd_min_math_peer_Q4
