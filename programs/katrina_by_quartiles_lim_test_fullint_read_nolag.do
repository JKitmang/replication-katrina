*===============================================================================
* FILE:     katrina_by_quartiles_lim_test_fullint_read_nolag.do
...
* PRODUCES: READING "no-lag" robustness rows of the Houston linear-in-means
*           tables (Appendix Tables 9 & 10; companion to Table 4).
*
* PURPOSE:  Same "limited test" of the linear-in-means peer-effect model for
*           HISD incumbents' READING achievement as fullint_read.do, but DROPS
*           the lagged baseline-score control (ltaks_sd_min_read_*) entirely.
*           Peer score (by native quartile) is instrumented with the evacuee
...
*     (built but NOT used as controls in the regressions below).
...
* ==== READING dependent variable ====
  ...
  // keep only natives with a non-missing lagged baseline reading score (sample definition)
...
by campus taks_sd_min_read_quartile: center  taks_sd_min_read taks_sd_min_read_peer_*  _k*  katrina_frac_grade_read_Q* , replace

foreach var of varlist   taks_sd_min_read taks_sd_min_read_peer_*  _k*  katrina_frac_grade_read_Q*  {
  replace `var' = c_`var'
}

* ---- Fully interact each control with native reading quartile (NO-LAG: _k* only) ----
local x 1
...
* ---- Estimation: OLS / reduced-form / 2SLS (NO lagged-score control), LIM test ----
	...
	*OLS  (reading peer-score coefficients by native quartile)
	reg taks_sd_min_read taks_sd_min_read_peer_* _k*  _g*, cluster(campus) nocons
	...
	*REDUCED FORM  (evacuee share by quartile -> outcome)
	reg taks_sd_min_read katrina_frac_grade_read_Q*   _k*    _g* if taks_sd_min_read_peer_Q1 != ., cluster(campus) nocons
	...
	*SECOND STAGE  (2SLS: peer score instrumented by evacuee share, by quartile)
	ivreg taks_sd_min_read  (taks_sd_min_read_peer_Q* = katrina_frac_grade_read_Q*)  _k*  _g*, cluster(campus) nocons
	test taks_sd_min_read_peer_Q1 = taks_sd_min_read_peer_Q2 = taks_sd_min_read_peer_Q3 = taks_sd_min_read_peer_Q4
