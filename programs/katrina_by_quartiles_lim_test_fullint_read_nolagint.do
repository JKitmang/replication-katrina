*===============================================================================
* FILE:     katrina_by_quartiles_lim_test_fullint_read_nolagint.do
* PROJECT:  Katrina's Children (Imberman, Kugler & Sacerdote, AER)
* ROLE:     Robustness/variant (Houston linear-in-means achievement test, READING)
* PRODUCES: READING "lag-not-interacted" robustness rows of the Houston linear-
*           in-means tables (Appendix Tables 9 & 10; companion to Table 4).
*
* PURPOSE:  Same "limited test" of the linear-in-means peer-effect model for
*           HISD incumbents' READING achievement as fullint_read.do, but the
*           lagged baseline-score control is KEPT and entered ONLY as a single
*           level term (ltaks_sd_min_read), NOT fully interacted with native
*           quartiles. Peer score (by quartile) is instrumented with the evacuee
*           share (by quartile); the `test` commands check equality of the four
...
* ---- Estimation: lag entered as single level term (ltaks_sd_min_read), NOT interacted ----
        *QUARTILES

	*OLS  (reading peer-score coefficients by native quartile)
	reg taks_sd_min_read taks_sd_min_read_peer_* _k* ltaks_sd_min_read  _g*, cluster(campus) nocons
	...
	*REDUCED FORM  (evacuee share by quartile -> outcome)
	reg taks_sd_min_read katrina_frac_grade_read_Q*   _k*  ltaks_sd_min_read  _g* if taks_sd_min_read_peer_Q1 != ., cluster(campus) nocons
	...
	*SECOND STAGE  (2SLS: peer score instrumented by evacuee share, by quartile)
	ivreg taks_sd_min_read  (taks_sd_min_read_peer_Q* = katrina_frac_grade_read_Q*)  _k* ltaks_sd_min_read _g*, cluster(campus) nocons
	test taks_sd_min_read_peer_Q1 = taks_sd_min_read_peer_Q2 = taks_sd_min_read_peer_Q3 = taks_sd_min_read_peer_Q4
