*===============================================================================
* FILE:     katrina_by_quartiles_va_campusgrade.do
* PROJECT:  Katrina's Children (Imberman, Kugler & Sacerdote, AER)
* ROLE:     Robustness/variant (Houston VA quartile peer-effects, campus x grade FE)
* PRODUCES: Appendix Tables 4 & 5 (and App 41 behavior battery) - the
*           CAMPUS-x-GRADE FIXED-EFFECTS row of the Houston grade-level
*           achievement robustness battery.
*
* PURPOSE:  Robustness check that absorbs CAMPUS-x-GRADE fixed effects (instead
*           of campus only), so identification comes from within-school across-
*           grade variation in the evacuee share. Estimates the grade-level
*           evacuee share (katrina_frac_grade) effect on incumbent math/read
*           scores, attendance and infractions, pooled and within each
...
*           absorbing campusgrade); feeds the campus x grade FE row of App Tables 4/5.
*
* KEY STEPS:
...
*   - Build campusgrade FE id; grade x year interactions.
*   - Pooled VA model and quartile-specific models, absorbing campusgrade.
*
...
*   (Header line says "limits to post-Katrina years" but this file does NOT
*    keep if year>=2005; that restriction is in the campusyear variant.)
*
...
* ---- Build campus x grade FE identifier ----
gen double campusgrade = campus*1000000 + grade*100

*GENERATE GRADE X YEAR INTERACTIONS AND SCHOOL DUMMIES
xi i.grade*i.year

* ---- Pooled VA model, absorbing campus x grade FE (App Tables 4/5 campusgrade pooled row) ----
*RUN LINEAR MODEL
  di ""
  ...
  foreach var of varlist taks_sd_min_math taks_sd_min_read {;
	areg `var'  katrina_frac_grade  l`var'_* female ethnicit_1-ethnicit_4 econdis_2-econdis_4 _I* if `var'_quartile !=., cluster(campus) absorb(campusgrade);

  };
  foreach var of varlist perc_attn infractions {;
	areg `var'  katrina_frac_grade  l`var'_* female ethnicit_1-ethnicit_4 econdis_2-econdis_4 _I* , cluster(campus) absorb(campusgrade);

  };
  # delimit cr


 * ---- Re-estimate within each baseline achievement quartile, campusgrade FE (App Tables 4/5 quartile rows) ----
 *LOOP OVER QUARTILES
  ...
  *LOOP OVER DEPENDENT VARIABLES;
	...
	foreach var of varlist taks_sd_min_math taks_sd_min_read {;
		areg `var' katrina_frac_grade  l`var'_* female ethnicit_1-ethnicit_4 econdis_2-econdis_4 _I* 
			if `var'_quartile == `quartile', cluster(campus) absorb(campusgrade);
