*===============================================================================
* FILE:     katrina_by_quartiles_va_grade.do
...
* PRODUCES: Appendix Table 3 (grade-level share VA quartile specification) -
*           the "grade" (school x grade evacuee share) variant of the Houston
*           grade-level achievement battery; main grade-level VA estimates.
*
* PURPOSE:  Estimates how the share of Katrina/Rita evacuees in a Houston
*           school-grade (katrina_frac_grade) affects incumbent students'
*           standardized TAKS math/read scores, attendance and infractions.
*           Following Hoxby & Weingarth (2005), interacts the evacuee share with
*           the incumbent's pre-Katrina (2004) achievement quartile to test for
*           boutique / bad-apple / shining-light peer models. Differs from
*           katrina_by_quartiles_va.do by using the GRADE-level (not campus)
*           evacuee share and by adding a single pooled model with quartile
*           interactions. Runs separately for elem and midhigh.
*
...
*           and its quartile interactions). outreg block at bottom is inactive.
*
* KEY STEPS:
...
*   - Grade x year interactions (xi i.grade*i.year) + school FE via areg.
*   - Pooled linear VA model on grade evacuee share.
*   - Pooled model with evacuee share x quartile interactions.
*   - Re-estimate within each baseline achievement quartile (1-4).
*
...
*   - Commented blocks reference year==2006, year>=2005 (Sept-2005 median
*     construction); update if active.
*
...
*RUN LINEAR MODEL
  ...
  di ""
  * ---- Pooled VA model: grade-level evacuee share, school FE (App Table 3 pooled row) ----
  # delimit ;
...
* ---- Single pooled model with evacuee-share x baseline-quartile interactions ----
*SINGLE MODEL WITH QUARTILE INTERACTIONS*

  di ""
  di "POOLED QUARTILE MODEL"
  di ""
  # delimit ;
  foreach var in "math" "read" {;

  *BUILD EVACUEE SHARE x BASELINE ACHIEVEMENT QUARTILE INTERACTIONS;
  forvalues quartile = 1/4 {;
    gen katrina_frac_grade_`var'_`quartile' = katrina_frac_grade*(taks_sd_min_`var'_quartile == `quartile') if taks_sd_min_`var'_quartile != .;
  };

	areg taks_sd_min_`var'  katrina_frac_grade_`var'_*  ltaks_sd_min_`var'_* female ethnicit_1-ethnicit_4 econdis_2-econdis_4 _I*, cluster(campus) absorb(campus);
 ...
 * ---- Re-estimate within each baseline achievement quartile (App Table 3 quartile rows) ----
 *LOOP OVER QUARTILES
...
* fractions by 2005 score-median (median-based peer composition) and saves
* katrina_medians.dta. Contains YEAR HARDCODEs (keep if year>=2005;
* median replaced if year==2006 to evaluate all evacuees on 2005 median)
* and PATH absolutes -- repoint if reactivated.
/*
