...
*           regressions after DROPPING the 2004 school year, creating an
*           "interrupted" panel. This guards against the estimates being driven
*           by anything specific to the immediate pre-Katrina year while still
*           using earlier years for the value-added lag controls.
*
...
*   - Drop year 2004 (the interrupted-panel restriction).
...
*   - drop if year == 2004  (the interrupted-panel cut)
...
*INTERUPPTED PANEL --> DROP 2004-05*
...
foreach grade in "elem" "midhigh"{
  ...
  * ---- INTERRUPTED-PANEL RESTRICTION: drop the 2004 school year ----
  *DROP 2004
  drop if year == 2004   // YEAR HARDCODE: drops the immediate pre-Katrina year to create the interrupted panel
