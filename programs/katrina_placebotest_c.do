*===============================================================================
* FILE:     katrina_placebotest_c.do
* PROJECT:  Katrina's Children (Imberman, Kugler & Sacerdote, AER)
* ROLE:     Robustness/variant (Houston placebo test, campus-level share)
* PRODUCES: Placebo columns of Table 3 (and the campus-level placebo robustness
*           via katrina_placebotest_grade companion).
*
* PURPOSE:  Falsification test: assigns each campus's ACTUAL 2005-06 evacuee share
*           to the PRE-Katrina years (2003-04 and 2004-05), then re-runs the
*           value-added regressions. Since no evacuees were present in those years,
*           a significant "effect" would signal that the evacuee share is correlated
*           with pre-existing campus trends rather than a causal peer effect.
*
...
* OUTPUTS:  /work/i/imberman/imberman/temp.dta  (intermediate: 2005 campus shares)
*           Regression output to log (no table file written here).
*
* KEY STEPS:
*   - Collapse 2005 data to campus-mean evacuee share -> temp.dta.
*   - Loop over grade level (elem, midhigh); rebuild pre-Katrina test-score lags.
*   - Keep only 2003-04 & 2004-05; merge the 2005 campus share onto those years.
*   - Set the placebo share to 0 in 2003 (so identification is the 2004 "as-if" share).
*   - Merge baseline achievement quartiles; areg with campus FE + grade*year.
...
*   - replace katrina_frac_campus = 0 if year == 2003  (placebo share zeroed in 2003)
...
* ---- Build the placebo share: actual 2005-06 campus evacuee share ----
...
*KEEP 2005 ONLY
keep if year == 2005   // YEAR HARDCODE: 2005-06 = treatment year; its actual evacuee share becomes the placebo share

collapse (mean) katrina_frac_campus, by(campus)
*PATH: repoint to your local globals.
...
foreach grade in "elem"  "midhigh"{;
  ...
  *MERGE IN KATRINA FRACTION DATA
  sort campus
  drop _merge
  drop katrina_frac_campus
  *PATH: repoint to your local globals.
  merge campus using /work/i/imberman/imberman/temp.dta, nokeep
  replace katrina_frac_campus = 0 if year == 2003   // YEAR HARDCODE: placebo share set to 0 in 2003; only 2004 carries the "as-if" 2005 share
  ...
  *LOOP OVER DEPENDENT VARIABLES;
  foreach subject of varlist taks_sd_min_math taks_sd_min_read {;
	...
	areg `subject' katrina_frac_campus l`subject'_* female ethnicit_1-ethnicit_4 econdis_2-econdis_4 _I* if `subject'_quartile != ., cluster(campus) absorb(campus);

  };
  foreach subject of varlist perc_attn infrac {;
    
	areg `subject' katrina_frac_campus l`subject'_* female ethnicit_1-ethnicit_4 econdis_2-econdis_4 _I*, cluster(campus) absorb(campus);
