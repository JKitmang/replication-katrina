*===============================================================================
* FILE:     quartile_analysis_revision_d.do
* PROJECT:  Katrina's Children (Imberman, Kugler & Sacerdote, AER)
* ROLE:     Analysis (workhorse Louisiana nonlinear "quartile" peer-effects file)
* PRODUCES: Tables 5 & 7 (main text) and a large battery of Appendix Tables
*           (App 12-13, 15-19, 21-22, 27-28, 30, 32-37, 39, 43 per the dofile
*           -> table map). Each estimation block below maps to a panel/column of
*           those tables (school- vs grade-level evacuee share, no-lag vs lag
*           sample, math vs ELA, all-students vs own-ability-quartile splits).
*
* PURPOSE:  Estimates the *structure* of peer effects from Hurricane Katrina/Rita
*           evacuees on incumbent ("native") Louisiana students. Sorts both
*           incumbents and evacuees into pre-Katrina achievement QUARTILES and
*           interacts the evacuee SHARE with those quartiles, so the design can
*           distinguish linear-in-means vs. monotonicity vs. boutique/ability-
*           grouping vs. bad-apple models. Runs the full specification battery
*           (school/grade/class evacuee share, FRP & median-Katrina trends,
*           discipline share, school-grade and school-year FE, enrollment
*           polynomials, sample restrictions) for both math and ELA.
*
...
* OUTPUTS:  outreg2 Excel/txt result files in /work/i/imberman/imberman/la_data/:
*           elem_school.xls, midhigh_school.xls, elem_grade.xls, midhigh_grade.xls
*           (no .dta saved; this is an estimation/output file).
*
* KEY STEPS:
*   - Load full LA panel; merge in alternative (2003-04 scale-based) standardized
*     scores, discipline microdata, and test-administrator (class) numbers.
*   - Build baseline achievement quartiles for math & ELA (own ability + peers).
...
*   - Run the quartile-interacted areg battery and outreg2 the results.
*
* HARDCODED YEARS / PATHS TO UPDATE FOR A NEW-YEARS REPLICATION:
*   - Standardization base year: scores standardized on 2003-04 means/SDs (see
*     header note below and the merged alternative_standardization.dta).
...
*   - discipline05 defined off year==2005; med_percent_katrina off year==2006.
*   - year!=2005 / year>=2006 subsample restrictions in robustness blocks.
*   - ABSOLUTE PATHS: every /work/i/imberman/imberman/la_data/... use/merge/
*     outreg2/rm path must be repointed to your local globals.
*
...
* ---- LOAD DATA & MERGE IN ALTERNATIVE STANDARDIZATION ----
...
* SCORE STANDARDIZATION: brings in mathSTD/elaSTD standardized on 2003-04 scale
* score means/SDs (see file header). PATH: repoint to your local globals.
...
* BASELINE ACHIEVEMENT QUARTILES: sort each student into within grade x year
* quartiles of pre-Katrina (0005) math/ELA achievement
...
drop if year == 2007 & grade_num == 4   // YEAR HARDCODE: drop uncovered grade-year
drop if year == 2007 & grade_num == 5   // YEAR HARDCODE: drop uncovered grade-year


* ---- IDENTIFY THE LAG SAMPLE (grades/years with a usable prior score) ----
gen lagsample = 0
replace lagsample = 1 if year > 2001 & grade_num == 10   // YEAR HARDCODE: grade 10 has lag from 2002 on
replace lagsample = 1 if year > 2003 & grade_num == 8    // YEAR HARDCODE: grade 8 has lag from 2004 on
...
* ---- GRADE x YEAR INTERACTION DUMMIES (gryr*) ----
drop gryr*
...
* BASELINE ACHIEVEMENT QUARTILES: own pre-Katrina math quartile (math_0004QUART)
...
* BASELINE ACHIEVEMENT QUARTILES: own pre-Katrina ELA quartile (ela_0004QUART)
...
***********************************
* ---- MERGE IN THE TEST ADMINISTRATOR DATA (used to define classrooms) ----
***********************************
capture drop _m
sort id year

* PATH: repoint to your local globals (TA numbers 2006-2009)
...
* ---- EXCLUDE EVACUATION-AREA DISTRICTS (Cameron & Calcasieu, hit by Rita) ----
...
* ---- DEFINE EVACUEE (RITA) SHARE AT SCHOOL x YEAR ----
* define percent Rita
...
* ---- REDEFINE PERCENT KATRINA (evacuee share at school x year, incl. Rita) ----
...
* school x year x grade evacuee share
...
replace katrina_number_grade = 0 if year <= 2005   // YEAR HARDCODE: no evacuees pre-2005-06
gen unit = 1
...
* ---- "BAD APPLE" SHARE: evacuee peers who were disciplined pre-Katrina ----
gen discipline05a=discipline_any if year==2005   // YEAR HARDCODE: baseline (pre-Katrina) discipline status
...
* ---- SCHOOL-SWITCHER FLAG (consecutive-year observations) ----
...
* ---- EVACUEE SHARE BY BASELINE QUARTILE — SCHOOL x YEAR LEVEL ----
* These Kfraction_* variables are the key RHS regressors: the fraction of the
* school-year that is an evacuee in each pre-Katrina achievement quartile.
...
* YEAR HARDCODE: evacuee-share-by-quartile forced to 0 pre-2005-06
...
* ---- EVACUEE SHARE/COUNT BY BASELINE QUARTILE — SCHOOL x YEAR x GRADE LEVEL ----
* Kfraction_*G = grade-level evacuee share by quartile (main grade-level RHS);
* Knumber_*G = evacuee counts; Tnumber_*G = total counts by quartile.
...
* YEAR HARDCODE: grade-level evacuee share/count by quartile = 0 pre-2005-06
...
* ---- ENROLLMENT/COUNT POLYNOMIAL TERMS (squared/cubed/quartic) ----
...
replace ela_lagyear = . if year > 2005     // YEAR HARDCODE: recompute lag-year only for post-Katrina obs
...
*===============================================================================
* ESTIMATION BATTERY BEGINS
* Each block re-runs the same quartile-interacted areg for math & ELA, no-lag
* (elementary) vs lag (mid/high) samples, all-students vs own-quartile splits.
* The "lincom" lines after each regression test peer-effect structure:
*   Q2-Q1, Q3-Q2, Q4-Q3, ... = MONOTONICITY tests
*   own-quartile vs each other = BOUTIQUE / ability-grouping test
*===============================================================================

**************
** Overall Table ***   --> SCHOOL-LEVEL evacuee share (Tables 5/7; App tables)
**************
...
* ---- Clear prior outreg2 output files (PATH: repoint to your local globals) ----
...
* ---- SCHOOL-LEVEL, ALL STUDENTS (not split by own quartile) ----
* Elementary = no-lag sample (grades<=5); outputs -> elem_school.xls
*Elementary (NO LAG)
...
****************************
** BY NATIVE LAGGED SCORE QUARTILE**   --> SCHOOL-LEVEL, split by incumbent's own baseline quartile
****************************
* For each own-quartile `quart' (BASELINE ACHIEVEMENT QUARTILES of the native),
* run linear (percent_katrina) then non-linear (Kfraction by quartile) specs.
...
*===============================================================================
*********GRADE LEVEL************
* Repeat the whole battery using GRADE-level evacuee share (Kfraction_*G).
* This is the main grade-level specification underlying Tables 5 & 7.
*===============================================================================
...
* ---- GRADE-LEVEL, ALL STUDENTS (not split by own quartile) ----
*Elementary (NO LAG)
...
****************************
** BY NATIVE LAGGED SCORE QUARTILE**   --> GRADE-LEVEL, split by incumbent's own baseline quartile
****************************
...
* ROBUSTNESS: add separate linear time trend for high- vs low-FRP schools
* (controls for differential trends correlated with poverty). Appendix robustness.
*===============================================================================

* ---- BASELINE (2005) median FRP split + FRP-specific trend ----
...
* ROBUSTNESS: separate trend for high- vs low-evacuee-share schools
* (App 23-24 territory / differential-trend robustness).
*===============================================================================
* ---- BASELINE (2006) median evacuee-share split + Katrina-specific trend ----
...
* BAD-APPLE MODEL: add percent_katrinaDISCIPLINE2 (share of evacuee peers who
* were disciplined pre-Katrina) alongside the quartile shares. Appendix table.
*===============================================================================
...
* CLASS-LEVEL specification using Kfraction_*C (school x year x TA classroom).
* No outreg2 here (results displayed only). Appendix robustness.
*===============================================================================
...
* ROBUSTNESS: school x year fixed effects, post-Katrina years only (App 27-28
* "campus-year" territory). Identification from within-school-year grade variation.
*===============================================================================

gen double sitecode_year = sitecode*100000 + year   // YEAR HARDCODE: encodes year into school-year FE id
* NOTE: estimation subsamples below use year >= 2006 (post-Katrina only)
...
* ROBUSTNESS: drop high-evacuee-share schools (outlier/leverage check, App table).
*===============================================================================
...
* ROBUSTNESS: drop the 2005 transition year (year!=2005 below). Appendix table.
*===============================================================================
...
* ROBUSTNESS: use a single linear lag (math_lag/ela_lag) rather than lag x
* years-since-lag interactions. App "lag not interacted" rows.
*===============================================================================
...
* ROBUSTNESS: keep only schools that actually received evacuees in 2005-06
* (treatment-on-the-treated / common-support check). App table.
*===============================================================================
...
* ROBUSTNESS: use evacuee COUNTS (Knumber_*G) instead of shares, controlling for
* a quartic in total enrollment/counts (Tnumber_*). App "total enrollment" table.
*===============================================================================
...
* ROBUSTNESS: school x grade fixed effects (App "campus-grade" table).
*===============================================================================
...
* ROBUSTNESS: school x year fixed effects, post-Katrina only (App "campus-year").
* Subsamples below use year >= 2006.
*===============================================================================
gen double school_year = sitecode*10000 + year   // YEAR HARDCODE: encodes year into school-year FE id
