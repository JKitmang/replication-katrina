*===============================================================================
* FILE:     quartile_analysis_revision_full_FE_b.do
* PROJECT:  Katrina's Children (Imberman, Kugler & Sacerdote, AER)
* ROLE:     Robustness/variant (Louisiana quartile peer effects with STUDENT FE)
* PRODUCES: Appendix Table 31 (LA by-quartile peer effects estimated with student
*           fixed effects instead of school fixed effects)
*
* PURPOSE:  Re-estimates the LA non-linear (by-quartile) peer-effects regressions
*           using student fixed effects (xtreg ..., fe, panel id = student id) so
*           that identification comes from within-student changes in the evacuee-
*           quartile composition of peers. Run overall and separately by the native
*           student's own baseline-achievement quartile, in math and ELA, with no
*           lagged dependent variable (FE absorbs the time-invariant ability term).
*
...
* OUTPUTS:  No saved dataset; xtreg results printed to the log -> App Table 31.
*
* KEY STEPS:
...
*   - Keep natives only; xi i.sitecode; compress; xtset id year (panel = student).
*   - Run xtreg ..., fe (student FE) overall, then by native quartile, math & ELA.
*
...
* ---- Declare student-year panel for student fixed effects (xtset id year) ----
xi i.sitecode
compress
xtset id year
...
xtset id year
* ---- STUDENT FE regressions, ALL natives (math then ELA) -> App Table 31 ----
*FULL SAMPLE (NO LAG)
...
* ---- STUDENT FE regressions, by native baseline quartile (loop 1..4) -> App Table 31 ----
****************************
