*===============================================================================
* FILE:     quartile_analysis_revision_full_b_tests_nobon.do
* PROJECT:  Katrina's Children (Imberman, Kugler & Sacerdote, AER)
* ROLE:     Analysis (Louisiana peer-effect model tests, NO Bonferroni)
* PRODUCES: Table 8 and Appendix Table 20 (formal tests of competing peer-effect
*           models for Louisiana: weak/strong monotonicity, invidious comparison,
*           ability-grouping/boutique, and the Duflo-et-al. bad-apple propositions)
*
* PURPOSE:  After estimating the by-quartile peer-effects regressions for the four
*           native baseline-achievement quartiles, this file stacks them in a SUR
*           (suest) system and runs joint Wald `test` commands that map onto each
*           theoretical peer-effects model. This is the "nobon" version: the mtest
*           multiplicity adjustment is UNcorrected (mtest, not mtest(b)). The sister
*           file quartile_analysis_revision_full_b_tests.do is identical but applies
*           the Bonferroni correction mtest(b).
*
...
* OUTPUTS:  No saved dataset; the suest + test results printed to the log ARE the
*           output reported in Table 8 / App Table 20.
*
* KEY STEPS:
*   - Identical data-prep backbone to the other LA quartile files: load FULL_SAMPLE,
*     merge standardized/discipline/TA data, build baseline quartiles, restrict
*     grades/years, define lag sample, redefine percent-Katrina (incl. Rita +
*     Cameron/Calcasieu), build Kfraction_*Q*G school*grade*year evacuee shares.
*   - Keep natives only; LIMIT to the lag sample.
*   - For each native quartile 1-4, run math & ELA regressions and estimates store.
*   - suest the four stored models (clustered by school) for math, then ELA.
*   - Run the battery of `test ..., mtest` commands implementing each peer model.
*
...
* ---- BASELINE ACHIEVEMENT QUARTILES: within-grade*year quartiles of math0005/ela0005 ----
...
* ---- Restrict to lag sample (middle/high grades with lagged dependent variable) ----
*LIMIT TO LAG SAMPLE
keep if lagsample == 1

* ---- School FE as explicit dummies (suest cannot use areg's absorbed FE) ----
**SUR CANNOT BE DONE WITH AREG SO NEED TO USE DUMMY VARIABLES
xi i.sitecode

* ---- Per-quartile regressions, stored for the SUR system (math`q', ela`q') ----
****************************
...
* ---- MATH: stack the four quartile models in a SUR system, cluster by school ----
*      All `test ..., mtest` blocks below report to Table 8 / App Table 20 (MATH panel).
*      "nobon" => mtest WITHOUT Bonferroni (sister file uses mtest(b)).
*MATH TESTS;
suest math1 math2 math3 math4, vce(cluster sitecode);
  ...
  * ---- TEST: joint significance of all 16 evacuee-quartile coefficients (MATH) ----
  *      (nobon: mtest, no Bonferroni)
  ...
  * ---- TEST: WEAK MONOTONICITY (MATH) -- Q1 share = Q4 share within each native quartile ----
  *WEAK MONOTONICITY ;
  ...
  * ---- TEST: STRONG MONOTONICITY 1 (MATH) -- adjacent-quartile equality (mtest) ----
  ...
  * ---- TEST: STRONG MONOTONICITY 2 (MATH) -- all four shares equal, per native quartile ----
  ...
  * ---- TEST: INVIDIOUS COMPARISON (MATH) -- higher-achieving peers hurt, lower-achieving help ----
  ...
  * ---- TEST: ABILITY GROUPING / BOUTIQUE (MATH) -- own quartile of peers beats higher quartiles ----
  *ABILITY GROUPING (BOUTIQUE/TRACKING) - YOUR OWN QUARTILE BEATS HIGHER QUARTILES;
   ...
   * ---- TEST: DUFLO ET AL. PROP 1 (MATH) -- own-quartile effect sign flips at median ----
   ...
   * ---- TEST: DUFLO ET AL. PROP 3 (MATH) -- f() monotone increasing in peer achievement ----
  ...
  * ---- TEST: DUFLO ET AL. constant-f() implications (MATH) -- single-crossing pattern ----
  test 	[math4_mean]Kfraction_mathQ4G	/* NEGATIVE = REJECT */
	[math3_mean]Kfraction_mathQ4G
...
* ---- ELA: same SUR + test battery as MATH above -> Table 8 / App Table 20 (ELA panel) ----
*ELA TESTS;
