*===============================================================================
* FILE:     quartile_analysis_revision_full_scale.do
* PROJECT:  Katrina's Children (Imberman, Kugler & Sacerdote, AER)
* ROLE:     Robustness/variant (Louisiana quartile peer effects, RAW SCALE scores)
* PRODUCES: Appendix Table 29 (LA by-quartile peer effects with UNSTANDARDIZED
*           scale-score outcomes instead of standardized scores)
*
* PURPOSE:  Re-estimates the LA non-linear (by-quartile) peer-effects regressions
*           using raw test scale scores (math_scaled / ela_scaled) as the dependent
*           variable rather than the within-grade*year standardized scores, to show
*           the results are not an artifact of the standardization. Run overall and
*           by native baseline quartile, separately for elementary (no lag) and
*           middle/high (lagged dependent variable) samples, in math and ELA.
*
...
*           alternative_scaled.dta              (RAW scale scores: *_scaled outcomes)
*           discipline_prepped_microdata.dta    (discipline records)
*           tanumbers06-09_prepped2.dta         (test-administrator / class IDs)
* OUTPUTS:  scale.xls   (regression output -> App Table 29)
*
* KEY STEPS:
*   - Same data-prep backbone as the other LA quartile files PLUS an extra merge of
*     alternative_scaled.dta to bring in raw scale-score outcomes (math/ela_scaled).
*   - Build baseline quartiles, restrict grades/years, define lag sample, redefine
*     percent-Katrina (incl. Rita + Cameron/Calcasieu), build Kfraction_*Q*G shares.
*   - Keep natives only.
*   - areg (school FE) on RAW scale scores: ALL then by native quartile; elementary
*     (no lag) and middle/high (with lags); math & ELA; outreg2 to scale.xls.
*
...
* ---- Merge in RAW SCALE scores (math_scaled / ela_scaled = App Table 29 outcomes) ----
...
* ---- RAW SCALE-SCORE regressions (areg, school FE), ALL natives -> App Table 29 ----
*      Outcome = math_scaled / ela_scaled (unstandardized); dec(0) since scale units.
********************************************************************
...
* App Table 29 -- elementary (grades<=5), math, ALL natives, raw scale score
...
* ---- RAW SCALE-SCORE regressions, by native baseline quartile (loop 1..4) -> App Table 29 ----
forvalues quart = 1/4 {
