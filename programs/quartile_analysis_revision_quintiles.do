*===============================================================================
* FILE:     quartile_analysis_revision_quintiles.do
* PROJECT:  Katrina's Children (Imberman, Kugler & Sacerdote, AER)
* ROLE:     Robustness/variant (Louisiana peer effects sorted by QUINTILES)
* PRODUCES: Appendix Table 36 (LA non-linear peer effects using achievement
*           QUINTILES instead of quartiles)
*
* PURPOSE:  Re-runs the LA non-linear peer-effects analysis splitting both natives
*           and evacuees into five baseline-achievement bins (quintiles, nq(5))
*           rather than four, to check the cross-bin pattern is robust to the number
*           of bins. Native standardized scores are regressed on the school*grade*
*           year share of evacuees in each quintile, separately by the native
*           student's own quintile, for elementary (no lag) and middle/high (lagged)
*           samples, in math and ELA.
*           NOTE the variable names keep the "QUART"/"Q" stems but now hold quintiles.
*
...
* OUTPUTS:  /work/i/imberman/imberman/la_data/quintile_split.xls -> App Table 36
*
* KEY STEPS:
*   - Same data-prep backbone as the quartile files but quantiles ... nq(5).
*   - Build Kfraction_*Q1G..Q5G school*grade*year evacuee shares (FIVE bins).
*   - Keep natives only.
*   - areg (school FE) on standardized scores by native quintile: elementary
*     (no lag) loop, then middle/high (lag sample) loop; math & ELA; outreg2.
*
* HARDCODED YEARS / PATHS TO UPDATE FOR A NEW-YEARS REPLICATION:
*   - use/merge/outreg paths: /work/i/imberman/imberman/la_data/... (repoint)
*   - commented Windows path: D:\School\Katrina\LA DOE\Revision
*   - alt standardization uses 2003-04 grade-4/8/10 means/SDs (see file header note)
...
*****SEPARATES BY QUINTILES INSTEAD OF QUARTILES****
...
*cd "D:\School\Katrina\LA DOE\Revision"   // PATH: repoint to your local globals (commented Windows path)
...
use /work/i/imberman/imberman/la_data/la_prepped_revisionFULL_SAMPLE.dta   // PATH: repoint to your local globals
* sample 3
...
* ---- Merge in standardized scores (score standardization: *STD vars; 2003-04 base) ----
...
* ---- BASELINE ACHIEVEMENT QUINTILES: within-grade*year QUINTILES of math0005/ela0005 (nq(5)) ----
...
bysort grade_num year: quantiles math0005, gen(mathQUART) nq(5) stable   // nq(5): QUINTILES (var name keeps QUART stem)
bysort grade_num year: quantiles ela0005, gen(elaQUART) nq(5) stable
...
gen lagsample = 0
replace lagsample = 1 if year > 2001 & grade_num == 10   // YEAR HARDCODE: g10 lag from 2002
replace lagsample = 1 if year > 2003 & grade_num == 8     // YEAR HARDCODE: g8 lag from 2004
...
* ---- BASELINE QUARTILES (alt source: 2004/2003 scores; note nq(4) here) for sorting ----
...
*      never in a Katrina/Rita district coded 0; Rita kids = evacuees; 0 pre-treatment. ----
***********************************************
***********************************************
***********************************************
...
* ---- Bad-apple measure: peer share of evacuees with a 2005 discipline record ----
...
* ---- PEER QUINTILE SHARES: fraction of all peers in each baseline quintile (school*year, 5 bins) ----
...
egen fraction_mathQ5=mean(mathQD5), by(district_code school_code year)
...
egen fraction_elaQ5=mean(elaQD5), by(district_code school_code year)
...
egen Kfraction_mathQ5=mean(mathQD5*katrina_sum), by(district_code school_code year)
...
egen Kfraction_elaQ5=mean(elaQD5*katrina_sum), by(district_code school_code year)
...
replace Kfraction_mathQ5=0 if year<=2005
...
replace Kfraction_elaQ5=0 if year<=2005
...
egen Kfraction_mathQ5G=mean(mathQD5*katrina_sum), by(district_code school_code year grade_num)
...
egen Kfraction_elaQ5G=mean(elaQD5*katrina_sum), by(district_code school_code year grade_num)
...
replace Kfraction_mathQ5G=0 if year<=2005
...
replace Kfraction_elaQ5G=0 if year<=2005
...
cap rm /work/i/imberman/imberman/la_data/quintile_split.txt
cap rm /work/i/imberman/imberman/la_data/quintile_split.xls
...
areg mathSTD Kfraction_mathQ1G Kfraction_mathQ2G Kfraction_mathQ3G Kfraction_mathQ4G Kfraction_mathQ5G free_lunchA male black hisp asian gryr* if mathQD`quart' == 1 & grade_num<=5, absorb(sitecode) cluster(sitecode)
outreg2 Kfraction_* using /work/i/imberman/imberman/la_data/quintile_split.xls, excel dec(2) ctitle("Math, Non-Linear, Quintile `quart', Elem")

areg elaSTD Kfraction_elaQ1G Kfraction_elaQ2G Kfraction_elaQ3G Kfraction_elaQ4G  Kfraction_elaQ5G free_lunchA male black hisp asian gryr* if elaQD`quart' == 1 & grade_num<=5, absorb(sitecode) cluster(sitecode)
outreg2 Kfraction_* using /work/i/imberman/imberman/la_data/quintile_split.xls, excel dec(2) ctitle("ELA, Non-Linear, Quintile `quart', Elem")
...
areg mathSTD math_lag_* Kfraction_mathQ1G Kfraction_mathQ2G Kfraction_mathQ3G Kfraction_mathQ4G  Kfraction_mathQ5G free_lunchA male black hisp asian gryr* if lagsample == 1 & mathQD`quart' == 1, absorb(sitecode) cluster(sitecode)
outreg2 Kfraction_* using /work/i/imberman/imberman/la_data/quintile_split.xls, excel dec(2) ctitle("Math, Non-Linear, Quintile `quart', Midhigh")

areg elaSTD ela_lag_* Kfraction_elaQ1G Kfraction_elaQ2G Kfraction_elaQ3G Kfraction_elaQ4G  Kfraction_elaQ5G free_lunchA male black hisp asian gryr* if lagsample == 1 & elaQD`quart' == 1, absorb(sitecode) cluster(sitecode)
outreg2 Kfraction_* using /work/i/imberman/imberman/la_data/quintile_split.xls, excel dec(2) ctitle("ELA, Non-Linear, Quintile `quart', Midhigh")
