...
* ROLE:     Data cleaning
* PRODUCES: Intermediate datasets taks.dta (and taks_a.dta) - the TAKS test-score
*           inputs to the master Houston merge in merge_c.do
*
* PURPOSE:  Appends the raw HISD TAKS exam files (split by year, language
*           English/Spanish, and lettered sub-files) into one dataset, harmonizes
*           the very inconsistent raw variable names into a common taks_* scheme,
*           cleans bad / missing scores, reshapes long by subject, and for
*           students with multiple sittings keeps BOTH a min-score and a mean-score
*           version (min is the preferred measure for initial-fail retakers),
*           collapses to one record per student-year-subject, then reshapes wide.
*
* INPUTS:   taks_2002.dta, taks_<year>_<lang>_<subset>.dta (2003-2006)
* OUTPUTS:  taks.dta  (primary, min/avg score versions, used by merge_c.do)
*           taks_a.dta (alternate version produced by the trailing block)
*
* KEY STEPS:
*   - Append all year/language/subset TAKS files; stamp year
*   - Rename ~40 raw fields to common taks_* names; backfill alt-named fields
*   - Drop bad ids; null out scores where met-min flag is missing
*   - Reshape long by subject; flag multi-sitting and multi-campus cases
*   - Collapse (min) and (mean) to one obs per id-year-subject; reshape wide
*
* NOTE: A trailing block after the first `save taks` (from the lone `f` line
*       onward) builds taks_a.dta; the file ends inside an unterminated `*/`
*       comment, so that tail is effectively commented out / scratch.
*
* HARDCODED YEARS / PATHS TO UPDATE FOR A NEW-YEARS REPLICATION:
*   - cd C:\D\Research\Charter\Houston\HISDdata\DataFiles\TAKS          (PATH)
*   - use taks_2002 ; gen year = 2002                                  (YEAR HARDCODE: base year)
*   - local year 2003 / 2004 / 2005 / 2006 blocks, each with its own
*     hand-coded language and subset (a/b/c/d) file lists              (YEAR HARDCODE)
*   - TAKS coverage begins 2002; no pre-2002 TAKS exists
*
...
set mem 400m
set more off
cd C:\D\Research\Charter\Houston\HISDdata\DataFiles\TAKS   // PATH: repoint to your local globals

* ---- Append all TAKS files (by year, language, lettered subset) ----
*GET ALL TAKS FILES INTO ONE DATASET
  use taks_2002        // YEAR HARDCODE: 2002 is the first TAKS year
  gen year = 2002      // YEAR HARDCODE

  *2003                                  // YEAR HARDCODE: 2003 file list (eng/span x a/b)
  local year 2003
  ...
  *2004                                  // YEAR HARDCODE: 2004 file list (eng a-d, span b/c)
  local year 2004
  foreach lang in "eng" {
  ...
  *2005                                  // YEAR HARDCODE: 2005 file list (eng/span x a/b/c)
  local year 2005
  foreach lang in "eng" "span" {
  ...
  *2006                                  // YEAR HARDCODE: 2006 file list (eng/span x a/b/c)
  local year 2006
  foreach lang in "eng" "span" {
    foreach subset in "a" "b" "c"{
      append using taks_`year'_`lang'_`subset'.dta
...
* ---- Harmonize raw variable names into common taks_* scheme ----
...
* ---- Clean data: fix met-min flags, null out bad scores, drop bad ids ----
*CLEAN DATA
...
* ---- Multi-sitting handling: keep both min-score and mean-score versions ----
...
* ---- Collapse to one obs per id-year-subject (min & mean), then reshape wide ----
 ...
 *RESHAPE BACK INTO WIDE
 ...
 compress
 sort id year
* ---- Save primary TAKS file (input to merge_c.do) ----
 save taks, replace


* ---- Trailing block: alternate taks_a.dta build (effectively scratch) ----
* NOTE: from the lone `f` below onward this is an alternate, single-score-per-
* sitting build saved as taks_a.dta; the file ends inside an unterminated `*/`,
* so this tail is effectively commented-out / auxiliary. Left unchanged.
f
